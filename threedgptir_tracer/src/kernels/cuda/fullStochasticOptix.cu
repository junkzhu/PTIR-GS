...
// fullStochastic forward path tracer (stage2): identical to referenceOptix.cu except
// the surface intersection uses stochastic single-Gaussian free-flight selection
// (rayIntersectStochastic) instead of alpha-compositing (rayIntersect). IS/AH programs
// are unchanged (shared by shadow rays).
